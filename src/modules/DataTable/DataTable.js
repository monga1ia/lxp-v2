...
import {priceFormat, numberReverseFormat, hexToRgb, htmlDecode, floatFormat} from "utils/Util";
...
class DataTable extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            page_number: props.config.page_number ? props.config.page_number : 1,
            selectedRow: null,
    ...
    render() {
        ...
        if (this.state.sort && !this.props.hasCustomSort) {
            if (this.state.sort.order.toLowerCase() === 'asc') {
            ...
            <div className="as-react-table react-datatable" id={(this.props.id) ? this.props.id + "-container" : ""}>
                ...
                <div className="row table-head asrt-table-head"
                    ...
                    <div className="col-md-5">
                        ...
                        {
                            this.props.config.second_add_button_left && this.props.secondButtonLeftClick
                                ?
                                <button onClick={(e) => this.props.secondButtonLeftClick()}
                                        className={this.props.config.second_button_style}>
                                        <span>
                                            {this.props.config.second_button_name}
                                        </span>
                                </button>
                                : null
                        }
                        ...
                        <div className="table_tools" style={style.table_tool}>
                            {
                                ...
                                <Portal
                                    ...
                                    openOnTriggerClick
                                    trigger={
                                        <button className="btn btn-info datatable-tools-buttons"
                            ...
                            }
                            {
                                ...
                                <button className="btn btn-info buttons-excel datatable-tools-buttons"
                                        tabIndex="0"
                            ...
                            }
                            {
                                ...
                                <button className="btn btn-info buttons-csv datatable-tools-buttons"
                                        tabIndex="0"
                                        aria-controls="configuration_tbl"
                                        title="Export to PDF"
                                        style={style.table_tool_btn}
                                ...
                                {(filterRecords.length) ? filterRecords.map((record, rowIndex) => {
                                        ...
                                        >
                                            {
                                                columnList.map((column, colIndex) => {
                                                    return that._renderTd(this.props.staticFirstRow ? defaultRowIndex : rowIndex,
                                                        colIndex, column, record, hasContextMenu, contextMenus, column?.showRecordClassName);
                                                })
                                            }
                                        </tr>
                                    )
                                    ...
                                    <tr>
                                        <td colSpan={columnList.length} align="center">
                                            <div className={"trigger-inner-container text-center"}>
                                                    ...
                                                    if (column.footerSum) {
                                                        ...
                                                        try {
                                                            ...
                                                            for (let f = 0; f < nonFilterAllRecords.length; f++) {
                                                                ...
                                                                if (filterRecord[column.key]) {
                                                                    if (typeof (filterRecord[column.key]) === 'string' && filterRecord[column.key].includes(',')) {
                                                                        total = total + Number.parseFloat(numberReverseFormat(filterRecord[column.key], ',', ''));
                                                                    } else if (typeof (filterRecord[column.key]) === 'string' && filterRecord[column.key].includes("'")) {
                                                                        total = total + Number.parseFloat(numberReverseFormat(filterRecord[column.key], "'", ''));
                                                                    } else if (typeof (filterRecord[column.key]) === 'string' && filterRecord[column.key].includes("%")) {
                                                                        isPercent = true;
                                                                        ...
                                                                        if ($.isNumeric(filterRecord[column.key])) {
                                                                            total = total + Number.parseFloat(filterRecord[column.key]);
                                                                        }
                                                                    } else if (typeof (filterRecord[column.key]) === 'number') {
                                                                        total = total + Number.parseFloat(filterRecord[column.key]);
                                                                    }
                                                        ...
                                                        return <td key={'footer_' + column.key}
                                                                   className={classText ? classText : ""}>
                                                            <div className={"trigger-inner-container"}>
                                                                <b>{floatFormat(total/nonFilterAllRecords.length)}{isPercent ? '%' : ''}</b></div>
                                                        </td>
                                                    } else {
                                                        return <td key={'footer_' + column.key}>
    ...
    _renderTd(rowIndex, colIndex, column, rowData, hasContextMenu, contextMenus, showRecordClassName = false) {
        let that = this;
        let classText = "";
        ...
        if (showRecordClassName) {
            let additionalClass = rowData[column.key + '_className'];
            let rowDataClassName = rowData['tdClassName'];
            if (additionalClass) {
                classText = classText + " " + additionalClass;
            }
            if (rowDataClassName) {
                classText = classText + " " + rowDataClassName;
            }

        }
        // if (recordColumnNames && recordColumnNames.length > 0
        //     && recordClassName) {
        //     if (recordColumnNames.indexOf(recordClassName) > -1) {
        //         classText = classText + " " + recordColumnNames[recordColumnNames.indexOf(recordClassName)];
        //     }
        // }
                            ...
                            <div className={"trigger-inner-container"}>
                                {
                                    this.props.staticFirstRow && colIndex === 0
                                        ...
                                        :
                                        this._renderColCell(column.colType, rowData, column, rowIndex)
                                }
                            </div>
                        </ContextMenuTrigger>
                        :
                        ...
                        >
                            {
                                contextMenus.map(function (obj, i) {
                                    return (
                                        <MenuItem key={i}
                                                  onClick={() => that.props.contextMenuItemClick(rowData.id, obj.key,rowData)}>
        ...
        switch (colType) {
            ...
            case 'input':
                return <Input
                    ...
                    value={colType === 'studentEmail' ? record.editedEmail : record.comment}
            ...
            case 'inlineedit':
                return <InlineEdit
                    ...
                    isDisabled={record.isDisabled ? true : false}
                />
            case 'html':
                if (column.clickableTd && record.clickable) {
                    return <div
                        ...
                        onClick={() => {
                            this.props.tdClick(column.key, record.id, record)
                        }}>
                    </div>
                } else {
                    return <div
            ...
            case 'numericedit':
                return <Input
                    ref={this.rowInputRef}
                    value={record[column.key] !== null ? record[column.key] : ''}
                    type="number"
                    style={{width: 95}}
    ...
    exportToExcel() {
                        ...
                        if (column.colType && column.colType === 'number') {
                            ...
                            if (column.textValueKey) {
                                ...
                                if (record[column.key].props && record[column.key].props.data) {
                                    ...
                                    let cellText = htmlDecode(record[column.key]);
                                    recordCol[column.text] = this.props.staticFirstRow && colIndex === 1 ? rowIndex : cellText;
                                }
                            }
                            colIndex++;
        ...
        if(rowIndex == (filterRecords.length + 1)){
            let footerRecordCol = {};
            for (let i = 0; i < this.state.filterColumns.length; i++) {
                let column = this.state.filterColumns[i];
                ...
                if (column && column.footerAvg && column.footerAvg === true) {
                    ...
                    for (let record of filterRecords) {
                        if (record[column.key]) {
                            if (typeof (record[column.key]) === 'string' && record[column.key].includes(',')) {
                                total = total + Number.parseFloat(numberReverseFormat(record[column.key], ',', ''));
                            } else if (typeof (record[column.key]) === 'string' && record[column.key].includes("'")) {
                                total = total + Number.parseFloat(numberReverseFormat(record[column.key], "'", ''));
                            } else if (typeof (record[column.key]) === 'string' && record[column.key].includes("%")) {
                                isPercent = true
                                total = total + Number.parseFloat(numberReverseFormat(record[column.key], "%", ''));
                            } else if (typeof (record[column.key]) === 'string') {
                                if ($.isNumeric(record[column.key])) {
                                    total = total + Number.parseFloat(record[column.key]);
                                }
                            } else if (typeof (record[column.key]) === 'number') {
                                total = total + Number.parseFloat(record[column.key]);
                            }
                        }
                    }
                    avg = (floatFormat(total / filterRecords.length))
                    if(isPercent){
                        avg = avg + '%'
                    }
                    footerRecordCol[column.text] = avg;
                }
                if (column && column.footerSum && column.footerSum === true) {
                    let total = 0;
                    for (let record of filterRecords) {
                        if (record[column.key]) {
                            if (typeof (record[column.key]) === 'string' && record[column.key].includes(',')) {
                                total = total + Number.parseInt(numberReverseFormat(record[column.key], ',', ''));
                            } else if (typeof (record[column.key]) === 'string' && record[column.key].includes("'")) {
                                total = total + Number.parseInt(numberReverseFormat(record[column.key], "'", ''));
                            } else if (typeof (record[column.key]) === 'string' && record[column.key].includes("%")) {
                                total = total + Number.parseInt(numberReverseFormat(record[column.key], "%", ''));
                                ...
                                if ($.isNumeric(record[column.key])) {
                                    total = total + Number.parseInt(record[column.key]);
                                }
                            } else if (typeof (record[column.key]) === 'number') {
                                total = total + Number.parseInt(record[column.key]);
                            }
                    ...
                    footerRecordCol[column.text] = total
                }
            }
        ...
        FileSaver.saveAs(data, this.props.config?.filename || fileName + fileExtension);
    ...
    exportToPDF() {
        ...
        for (let column of this.state.filterColumns) {
            if (!column.hide && 
                ...
                if(column.colType === 'html'){
                    sTable += "<th>" + (column.text ? column.text : '') + "</th>";
                }else{
                    sTable += "<th>" + (column.text ? column.text : '')+ "</th>";
                }
            }
        ...
        for (let i in filterRecords) {
            ...
            for (let column of this.state.filterColumns) {
                if (!column.hide && 
                    ...
                    if(column.colType === 'html'){
                        if(column.textValueKey){
                            sTable += "<td>" + (this.props.staticFirstRow && colIndex === 1 ? rowIndex : record[column.textValueKey]) + "</td>";
                        }else{
                            sTable += "<td></td>";
                        }
                    }else{
        ...
        win.document.write('<title>' + this.props.config?.filename + '</title>');
        win.document.write(style);
        win.document.write('</head>');
        win.document.write('<body>');
        win.document.write('<h2>' + this.props.config?.filename + '</h2>');
