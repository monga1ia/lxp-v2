...
const PaginationLinks = ({ paginationProps }) => {
    ...
    return (
        <>
            ...
            {pagesCount > 1 && (
                <>
                    <div className={`pagination-link d-flex flex-wrap py-2 mr-3 ${disabledClass}`}>
                        <a
                            onClick={() => handleFirstPage(paginationProps)}
                            className={`btn btn-icon btn-sm btn-pagination me-1 ${isFirst ? 'disabled' : ''}`}
                        >
                        ...
                        {page > 1 && (
                            <a className="btn btn-icon btn-sm border-0 btn-white btn-pagination me-1">
                                ...
                            </a>
                        )}
                        {pages.map((p) => (
                            <a
                                ...
                                className={`btn btn-icon btn-sm btn-white border-0 btn-white btn-pagination ${
                        ...
                        {page < pagesCount && (
                            <a className="btn btn-icon btn-sm border-0 btn-white btn-pagination">
                                ...
                            </a>
                        )}
                        <a
                            onClick={() => handleNextPage(paginationProps)}
                            className={`btn btn-icon btn-sm btn-pagination ms-1 ${isLast ? 'disabled' : ''}`}
                        >
