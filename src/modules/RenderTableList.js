    ...
    return (
      <div className="row">
        <div className="col-12" style={{ display: "flex" }}>
          ...
          <div className="col-9">
            <table>
              <thead style={{ lineHeight: lineHeight || 1 }}>
            ...
            <thead style={{ lineHeight: lineHeight || 1 }}>
              {stringArray && stringArray.length > 0
                ? stringArray.map((label, index) => {
                    return (
                      <tr key={"key_" + index}>
                        <td className="text-right" style={leftStyle}>
                          {label.name}
                        </td>
                        <th className="pl-3" style={rightStyle}>
                          {label.value}
                        </th>
                      </tr>
                    );
                      ...
                      <tr key={"key_" + index} className="pt-3">
                        ...
                        >
                          <input
                            ...
                            onChange={(e) => {
                              handleInputChange(e, label.code, index);
                            }}
                            value={inputValue}
                          />
                          {label.isButton ? (
                            <div>
                              <button
                                ...
                                type="button"
                                onClick={() =>
                                  handlerInputButtonClick(label.code)
                                }
                    ...
                    return reverse == "all" ? (
                      label.hidden ? null : (
                        <tr key={"key_" + index} style={label.style}>
                          <th className="text-right" style={leftStyle}>
                            {label.name}
                          </th>
                          <th className="pl-3" style={rightStyle}>
                            {label.value}
                        ...
                        <tr key={"key_" + index} style={label.style}>
                          ...
                          {label.editable ? (
                            isEdit ? (
                              ...
                              <td className="pl-3" style={rightStyle}>
                                {label.value}
                              </td>
                            )
                          ) : (
