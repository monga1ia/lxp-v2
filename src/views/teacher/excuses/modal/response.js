...
// import {teacherExcuseRequestInfo} from 'Utilities/url'
...
const response = ({onClose, onSubmit, id, status}) => {
    ...
    // useEffect(() => {
    //     setLoading(true)
    //     fetchRequest(teacherExcuseRequestInfo, 'POST', {request: id, menu: 'teacher'})
    //         .then((res) => {
    //             if (res.success) {
        ...
        >
            <Modal.Header closeButton={!showAttachment} style={{ padding: '1rem' }}>
                <Modal.Title className="modal-title d-flex flex-row justify-content-between w-100">
                    {translations(locale)?.absent?.response}
                </Modal.Title>
                {
                    showAttachment &&
                        <button 
