...
const Question = ({
            ...
            <div className="d-flex flex-row question-table-style">
                ...
                <div className="w-100 row">
                    <div className="d-flex flex-row align-content-center align-items-center justify-content-between mb-4">
                        ...
                        {
                            showContextMenu && contextMenus?.length > 0 &&
                            <Button
    ...
    const submitReturn = (modalParams) => {
        let params = {
        ...
        onApprovalHandler(params)
        onModalCloseReturn()
    }

    const handlerUnpublishButton = () => {
        setShowReturnModal(true)
    }

    const onModalCloseReturn = () => {
        setShowReturnModal(false)
    }

    const handlerPublishButton = (activeList) => {
        let params = {
            school: selectedSchool?.id,
            question: id,
            approve: 1
        ...
        onApprovalHandler(params, activeList)
    }

    const checkFooterArea = (isOrganization, activeList) => {
        if (!isOrganization && activeButtonCode == 'NOT_APPROVED') {
            return null
        } else if (!isOrganization && activeButtonCode == 'DECLINED') {
            return (
                <Card.Footer style={{ padding: '1.3rem' }}>
                    <div className="d-inline-block">
                        <p className="mb-0">
                            {t('adminQuestion.label.returned') + ': ' + questionObj?.approvalFirstName + ' | ' + questionObj?.approvalDate?.date.substring(0, 19)}
                        </p>
                        <p className="mb-0">
                            {questionObj?.declineDescription || ''}
                        </p>
                    </div>
                </Card.Footer>
            )
        } else if (!isOrganization && activeButtonCode == 'APPROVED') {
            return (
                <Card.Footer style={{ padding: '1.3rem' }}>
                    <div className="d-flex flex-row">
                        {t('adminQuestion.label.published') + ': ' + questionObj?.approvalFirstName + ' | ' + questionObj?.approvalDate?.date.substring(0, 19)}
                    </div>
                </Card.Footer>
            )
        } else if (isOrganization && activeButtonCode == 'NOT_APPROVED') {
            return (
                <Card.Footer style={{ padding: '1.3rem' }}>
                    <div className="d-flex flex-row">
                        {
                            !questionObj.isApproved && <>
                                <button className="btn outline-success-custom-1 text-uppercase" onClick={() => handlerPublishButton(activeList)}>
                                    {t('adminQuestion.label.publish')}
                                </button>
                                <button className="btn outline-danger-custom-1 text-uppercase ml-2" onClick={() => handlerUnpublishButton(activeList)}>
            ...
            return (
                <Card.Footer style={{ padding: '1.3rem' }}>
                    <div className="d-flex flex-row justify-content-between">
                        ...
                        <div>
                            <button className="btn outline-danger-custom-1 text-uppercase" onClick={() => handlerUnpublishButton(activeList)}>
                                {t('adminQuestion.label.return')}
                            </button>
                        </div>
                    </div>
    ...
    return (
        <Card className={`mb-3 ${className}`}>
            ...
            {checkFooterArea(isOrganizationUser, list)}
