...
import { examQuestions, examCreateFromBluePrint, examRemoveFromBluePrint, examPublish, examCheckExamPdf } from 'utils/fetchRequest/Urls';
...
export default function QuizDetails({
    ...
    const printButtonHandler = () => {
        ...
        fetchRequest(examCheckExamPdf, 'POST', params)
            .then((res) => {
                if (res.success) {
                    setPrintQuestions(res.questions || [])
        ...
        fetchRequest(examPublish, 'POST', params)
            .then((res) => {
                if (res.success) {
                    ...
                    setIsPublish(isPublish)
                    history.push({
                        pathname: '/online-exam/print-exam',
                    })
                } else {
                    showMessage(res.message)
                }
    ...
    return (
        <>
            ...
            <div className="text-center" style={{ borderTop: '1px solid #d8d8d8', padding: 24 }}>
                ...
                <Button 
                    className="cursor-pointer save-button" 
                    variant='empty' 
                    onClick={onSubmit}
                >
                ...
                <Button
                    style={{ background: '#3EBFA3', width: 145, textTransform: 'uppercase'}}
                    className='save-button secondary ml-3'
                    variant='empty'
                    onClick={printButtonHandler}
                >
                    <span style={{ color: '#000' }}>{t("common.print")}</span>
                </Button>
            </div>
            {
                showPrintModal &&
