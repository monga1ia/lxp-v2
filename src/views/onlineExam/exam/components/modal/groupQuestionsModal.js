...
import { examGroupQuestionCreate, examGroupQuestionEdit } from 'utils/fetchRequest/Urls';
...
export default function GroupQuestionsModal({ 
    ...
    const init = (params) => {
        setLoading(true)
        fetchRequest(examGroupQuestionCreate, 'POST', params, true, true)
            .then((res) => {
                setLoading(false)
                if (res.success) {
                    const { questions = [] } = res

                    setQuestionOptions(questions.map(question => ({
    ...
    const saveButtonClick = () => {
        ...
        let url = examGroupQuestionCreate
