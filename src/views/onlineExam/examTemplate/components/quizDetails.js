...
import { examTemplateQuestions, examTemplateQuestionCreateFromBluePrint, examTemplateQuestionRemoveFromBluePrint, examTemplateQuestionSaveBluePrint, examTemplatePublish } from 'utils/fetchRequest/Urls';
...
export default function QuizDetails({
    ...
    const [selectedVariantId, setSelectedVariantId] = useState(location?.state?.templateVariantId || null);
    ...
    const [examCount, setExamCount] = useState(true)
    ...
    const init = (params, variantId = null, isChildTopic = false, index = null) => {
        setLoading(true)
        fetchRequest(examTemplateQuestions, 'POST', params)
            .then((res) => {
                if (res.success) {
                    const { method, variants = [], gradeId = null, subjectId = null, parentTopics = [], childTopics = [], difficulties = [], qTypes = [], blueprints = [], attributes = [], isPublish = false} = res
                    ...
                    if(isChildTopic){
                        ...
                        if(variantId && variantId == selectedVariantId){
    ...
    const getVariantIdAndTabIndex = () => {
        let examTemplateVariantStrings = secureLocalStorage.getItem(examTemplateTabIndex)

        if(examTemplateVariantStrings){
            let textVariantArray = examTemplateVariantStrings.split('_');
        ...
        secureLocalStorage.removeItem(examTemplateTabIndex)
    }
    ...
    const publishButtonHandler = () => {
        if(method == 'BLUEPRINT'){
            ...
            if(!error){
                ...
                setLoading(true)
                fetchRequest(examTemplateQuestionSaveBluePrint, 'POST', params)
                    .then((res) => {
                        if (res.success) {
            ...
            setLoading(true)
            fetchRequest(examTemplatePublish, 'POST', params)
                .then((res) => {
                    if (res.success) {
                        const { isPublish = false } = res
    ...
    const callVariantInit = (variantId, page, selectedTabIndex) => {
        if(page){
            let params = {
                school: selectedSchool.id,
                template: id,
    ...
    const onTabChange = (index, data) => {
        ...
        secureLocalStorage.setItem(examTemplateTabIndex, 'template_' + id + '_variant_' + data.id)
    ...
    const saveQuestionRow = (index) => {
        ...
        setLoading(true)
        fetchRequest(examTemplateQuestionCreateFromBluePrint, 'POST', params)
            .then((res) => {
                if (res.success) {
    ...
    const onDelete = () => {
        ...
        setLoading(true)
        fetchRequest(examTemplateQuestionRemoveFromBluePrint, 'POST', params)
            .then((res) => {
                if (res.success) {
    ...
    return (
        <>
            ...
            {
                method == 'VARIANT' && selectedGradeId && selectedSubjectId &&
                    <TabComponent
                        selectedTabIndex={selectedTabIndex}
            ...
            <div className="text-center" style={{ borderTop: '1px solid #d8d8d8', padding: 24, maxHeight: 83 }}>
                <Button
                    className='cancel-button pr-2'
                    variant='link'
                    onClick={cancelButtonHandler}
