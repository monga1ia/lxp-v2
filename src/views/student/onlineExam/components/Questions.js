...
const Questions = () => {
    ...
    const onAnswerClear = (question) => {
        ...
        setLoading(true)
        fetchRequest('api/student/exam/clear-answer', 'POST', { exam: id, question, page: selected })
            .then(res => {
                if (!res.success) {
                    message(res.message)
                }
                ...
                returnNoAnswerList(exam, res?.pages || [])
                setLoading(false)
            })
            .catch(e => {
    ...
    const onSubmitAnswer = (question, answer, value, clear = 0) => {
        setLoading(true)
        fetchRequest('api/student/exam/question-answer', 'POST', { exam: id, question, answer, value, clear, page: selected })
            .then(res => {
                if (!res.success) {
                    message(res.message)
                }
    ...
    const fetchInit = (page = 1) => {
        setLoading(true)
        fetchRequest('api/student/exam/questions', 'POST', { exam: id, page })
            .then(res => {
                if (res.success) {
                    setExam(res?.exam || null)
                    ...
                    returnNoAnswerList(res?.exam, res?.pages)
                    if (res?.remaining > 0) {
    ...
    const returnNoAnswerList = (examObj = null, list = []) => {
        let tempList = []
        ...
        if(examObj && examObj?.perPageSize == 1){
            if(list && list.length > 0){
                for(let i = 0; i < list.length; i++){
                    if(!list[i].completed){
                        tempList.push(list[i].page)
                    }
                }
            }
        } else {
            if(list && list.length > 0){
                for(let i = 0; i < list.length; i++){
                    if(list[i].notCompleted && list[i].notCompleted.length > 0){
                        let notCompleted = list[i].notCompleted;

                        for(let n = 0; n < notCompleted.length; n++){
                            tempList.push(notCompleted[n])        
                        }
                    }
                }
            }
        ...
        setPageCounter(tempList.sort((a, b) => a > b ? 1 : -1)) 
    }
            ...
            return pageList.map((obj, index) => {
                return (
                    ...
                    >
                        ...
                        {obj.completed ? <CheckIcon /> : <div style={{ width: 20 }} />}
                    </Button>
                )
            })
        }
    ...
    const Question = ({ list = [] }) => {
        return list.map((obj, index) => {
            return <div key={index} className='card-alternate-student-main mb-2 question-table-style'>
                ...
                </div>
                {
                    obj.parentQuestion
                        ...
                        <div className='card-alternate mb-2'>
                            <div dangerouslySetInnerHTML={{ __html: obj.parentQuestion.content }} />
                            {
                                obj.parentQuestion.files && obj.parentQuestion.files.length > 0
                                    ?
                                    <div className='col-12 text-center'>
                                        {
                                            obj.parentQuestion.files.map((file, index) => {
                                                ...
                                                return <Image key={index} src={file.path} style={{ resize: 'block', height: '120px', marginLeft: '1.2rem' }} />
                                            })
                                        }
                                    </div>
                                    : null
                ...
                }
                {
                    obj.userAnswers
                        ?
                        <Button variant='outline-primary' className='mt-3' onClick={() => onAnswerClear(obj.qId)}>
                            <XCircle />
                    ...
                    if (cloneQuestionList[i].answers && cloneQuestionList[i].answers.length > 0) {
                        ...
                        for (let a = 0; a < answers.length; a++) {
                            if (answerId == answers[a].id) {
                                if (!answers[a].qId) {
                                    setLoading(true)
                                    fetchRequest('api/student/exam/question-answer', 'POST', { exam: id, question: questionId, answer: answerId, value: item.id })
                                        .then(res => {
                                            if (res.success) {
                ...
                if (questionId == cloneQuestionList[i].qId) {
                    if (cloneQuestionList[i].answers && cloneQuestionList[i].answers.length > 0) {
                        ...
                        for (let a = 0; a < answers.length; a++) {
                            if (answerId == answers[a].id) {
                                setLoading(true)
                                fetchRequest('api/student/exam/clear-answer', 'POST', { exam: id, question: questionId, answer: answerId, value: answers[a].qId })
                                    .then(res => {
                                        if (res.success) {
        ...
        <div>
            ...
            <Row className='m-0'>
                        ...
                        <Card.Body className='p-3'>
                            <div className='d-flex flex-row align-items-center'>
                                ...
                                <div className='d-flex flex-column ml-2'>
                                    ...
                                    {
                                        remaining > 0 && <Counter time={remaining} onFinish={onFinish} />
                                    }
                ...
                </Col>
                {
                    selected
                        ?
                        <Col md={9} className='background-white' style={{ padding: 24 }}>
                            <Question
                                list={questionList}
                            />

                            <div className='d-flex flex-row justify-content-between border-top mt-3' style={{padding: '1rem 1rem 0 1rem'}}>
