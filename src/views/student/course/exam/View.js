...
const View = () => {
    ...
    const AnswerOptions = ({ list = [], match = [], answer = null, typeCode = '', question = null }) => {
        if (typeCode == 'TEST') {
            return list.map((obj, index) => {
                            ...
                            <div className='col-12'>
                                {
                                    list.map((mAnswer, index) => {
                                        ...
                                        return (
                                            <div 
                                                key={'mAnswer_' + index} 
                                                className='d-flex justify-content-between mb-1 student-exam-report-dropzone'
                                                style={{
                                                    padding: '0 100px', 
                                            ...
                                            >
                                                ...
                                                <div className='' style={mAnswer['userValue'] != mAnswer['valueId'] ? {position: 'relative', left: 94} : {}}>
                                                    <Dropzone
                                                        key={'answer_' + index}
        ...
        <>
            <Row className='mr-0'>
                <Col md={3} className='pl-0 mt-3' style={{ position: 'relative' }}>
                        ...
                        <Card.Body className='p-2'>
                            <div className='d-flex flex-row align-items-center'>
                                ...
                                <div className='d-flex flex-column'>
                                    ...
                                    <h5 className='font-bold'>{item?.takenScore || 0} | {takenPercent(item?.takenScore, item?.totalScore)}%</h5>
                                </div>
                            </div>
                        </Card.Body>
                    ...
                    <Card className='mt-2 ml-3 student-exam-report-col-3' style={{ overflow: 'hidden' }}>
                        <Card.Body className='p-2'>
                ...
                <Col md={9} className='background-white' style={{ overflow: 'hidden', maxHeight: '100%' }}>
                    <div className="student-page-title-container student-exam-report-back-button mb-3">
                        <Button variant='empty' onClick={() => onHandlerBack()}>
                            <CsLineIcons
                                stroke='#212529'
                                icon="arrow-left"
                                className="mr-2"
                    ...
                    <h4 className="mb-3 text-primary ml-5 font-bold student-exam-report-header">{item?.name}</h4>
                    <Grid container spacing={2} className='mb-3'>
                        <Grid className='student-exam-report-grid-items' item md={3}>
                            <div className='card-alternate mb-2'>
                                <h5 className='text-black font-bold'>{t('onlineExam.correctAnswers')}</h5>
                                <div className='d-flex flex-row justify-content-end w-100 mt-3'>
                                    <h5 className=' m-0 font-bold' style={{ color: '#63C132' }}>{item?.correctCount}</h5>
                                </div>
                            </div>
                        </Grid>
                        <Grid className='student-exam-report-grid-items' item md={3}>
                            <div className='card-alternate mb-2'>
                                <h5 className='text-black font-bold'>{t('onlineExam.inComplete')}</h5>
                                <div className='d-flex flex-row justify-content-end w-100 mt-3'>
                                    <h5 className='m-0 font-bold' style={{ color: '#FB8500' }} >{item?.incompleteCount}</h5>
                                </div>
                            </div>
                        </Grid>
                        <Grid className='student-exam-report-grid-items' item md={3}>
                            <div className='card-alternate mb-2'>
                                <h5 className='text-black font-bold'>{t('onlineExam.wrongAnswers')}</h5>
                                <div className='d-flex flex-row justify-content-end w-100 mt-3'>
                                    <h5 className='m-0 font-bold' style={{ color: '#FF003D' }}>{item?.wrongCount}</h5>
                                </div>
                            </div>
                        </Grid>
                        <Grid className='student-exam-report-grid-items' item md={3}>
                            <div className='card-alternate mb-2'>
                                <h5 className='text-black font-bold'>{t('onlineExam.noAnswer')}</h5>
                                <div className='d-flex flex-row justify-content-end w-100 mt-3'>
                                    <h5 className='m-0 font-bold' style={{ color: '#091E42' }}>{item?.noAnswerCount}</h5>
                                </div>
                            </div>
                        </Grid>
                    </Grid>
                            ...
                            <div className="d-flex justify-content-between">
                                {
                                    isPrev
                                        ?
                                        <div>
                                            <button type='button' className="btn btn-primary" onClick={() => onHandlerItem(prevDetail, activeParentIndex, parseInt(activeIndex.split('_')[1]) - 1)}>{t('onlineLesson.prev')}</button>
                                        </div>
                                        :
                                        <div />
                                }
                                {
                                    isNext
                                        ?
                                        <div>
                                            <button type='button' className="btn btn-primary" onClick={() => onHandlerItem(nextDetail, activeParentIndex, parseInt(activeIndex.split('_')[1]) + 1)}>{t('onlineLesson.continue')}</button>
                                        </div>
                                        :
                                        <div />
                                }
                    ...
                    {
                        !isDescription && componentType == 'parent' && syllabuses && syllabuses.length > 1 &&
                        <Card.Footer style={{ padding: '1rem 2rem' }} >
                            <div className="d-flex justify-content-between">
                                {
                                    isPrev
                                        ?
                                        <div>
                                            <button type='button' className="btn btn-primary" onClick={() => onHandlerItem(prevDetail, parseInt(activeParentIndex - 1), null, 'parent')}>{t('onlineLesson.prev')}</button>
                                        </div>
                                        :
                                        <div />
                                }
                                {
                                    isNext
                                        ?
                                        <div>
                                            <button type='button' className="btn btn-primary" onClick={() => onHandlerItem(nextDetail, parseInt(activeParentIndex + 1), null, 'parent')}>{t('onlineLesson.nextChapter')}</button>
                                        </div>
                                        :
                                        <div />
                                }
