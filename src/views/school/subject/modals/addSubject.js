...
import { schoolSubjectCreate } from 'utils/fetchRequest/Urls';

const AddSubjectModal = ({ onClose, onSubmit, curriculumId, curriculumName }) => {
    ...
    const subjectFields = (subjectTypes = [], grades = [], teachers = []) => {
        return [
            {
                key: 'curriculum',
                type: 'text',
    ...
    const loadData = (params = {}) => {
        setLoading(true)
        fetchRequest(schoolSubjectCreate, 'POST', params)
            .then((res) => {
                if (res.success) {
                    setGradeList(res?.grades)
                    setSubjectTypeList(res?.subjectTypes)
                    setTeacherList(res?.teachers)
                    formRef?.current?.updateFields && formRef.current?.updateFields(subjectFields(res?.subjectTypes, res?.grades, res?.teachers));
                } else {
                ...
                <div className="row">
                    <div className="col">
                        <div className="form-group m-form__group row mb-0">
                            <Forms
                                ref={formRef}
                                fields={subjectFields(subjectTypeList, gradeList, teacherList)}
                            />
                        </div>
